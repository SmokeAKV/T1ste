...
async function youtube(link) {
    try {
        ...
        const url = ytIdRegex.exec(link)[1]; // Agora a variável 'link' é usada corretamente
        const headerss = {
            "User-Agent": "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/91.0.4472.124 Safari/537.36",
        ...
        // Resto do código permanece igual...
                const analyzeData = await axios.post('https://www.y2mate.com/mates/en68/analyze/ajax', new URLSearchParams({
            'url': `https://www.youtube.be/${url}`,
            'q_auto': 0,
            'ajax': 1
        }), { headers: headerss });

        if (!analyzeData.data.result) {
            throw new Error('Failed to analyze the YouTube video.');
        }

        const $ = cheerio.load(analyzeData.data.result);
        ...
        const idMatch = /var k__id = "(.*?)"/.exec(analyzeData.data.result);
        ...
        const convertData = await axios.post('https://www.y2mate.com/mates/en68/convert', new URLSearchParams(Object.entries(configs)), { headers: headerss });

        if (!convertData.data.result) {
            throw new Error('Failed to fetch video link.');
        }

        const $convert = cheerio.load(convertData.data.result);
        ...
        const mp3ConvertData = await axios.post('https://www.y2mate.com/mates/en68/convert', new URLSearchParams(Object.entries(configss)), { headers: headerss });

        if (!mp3ConvertData.data.result) {
            throw new Error('Failed to fetch audio link.');
        }

        const $mp3Convert = cheerio.load(mp3ConvertData.data.result);
        const audio = $mp3Convert('div > a').attr('href');
