...
const makeSocket = (config) => {
    ...
    /** send a raw buffer */
    //const ws = new WebSocket('wss://example.com/socket'); // Certifique-se de inicializar seu WebSocket

// Adiciona manipuladores de eventos para monitorar o estado da conexão
    ...
    try {
        ...
        await (0, Utils_1.promiseTimeout)(connectTimeoutMs, async (resolve, reject) => {
            try {
                ...
                reject('Connection closed: ' + error);
            }
        });
    } catch (error) {
...
/*const sendRawMessage = async (data) => { if (!ws.isOpen) { throw new boom_1.Boom('Connection Closed', { statusCode: Types_1.DisconnectReason.connectionClosed }); } const bytes = noise.encodeFrame(data); await (0, Utils_1.promiseTimeout)(connectTimeoutMs, async (resolve, reject) => { try { await sendPromise.call(ws, bytes); resolve(); } catch (error) { reject('fechou' + error); } }); };*/


// Exemplo de chamada da função (assegure-se de que a conexão está aberta)
