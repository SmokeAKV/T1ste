...
//const { delayCancellable } = require('algum-modulo'); // Substitua por seu módulo real
const DisconnectReason = { timedOut: 408 }; // Certifique-se de que isso está correto
...
async function promiseTimeout(ms, promise) {
    ...
    return new Promise((resolve, reject) => {
        const timeoutPromise = delay.then(() => {
            const error = new Boom('Timed Out', {
                statusCode: DisconnectReason.timedOut,
                data: { stack }
            });
            console.error("Deu erro aqui no time");
            reject(error);
        }).catch(reject);

        const wrappedPromise = new Promise(promise);
        ...
        // Resolve ou rejeita a promessa original e limpa o timeout
        Promise.race([timeoutPromise, wrappedPromise])
            .then(resolve)
            .catch(reject);
    });
}
...
// Função delayCancellable de exemplo
