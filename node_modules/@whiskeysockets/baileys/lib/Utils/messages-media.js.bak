...
const getStream = async (item, opts) => {
    ...
    if (item && item.url && typeof item.url === 'string') {
        if (item.url.startsWith('http://') || item.url.startsWith('https://')) {
            console.log(`Fetching remote stream from: ${item.url}`);
            return { stream: await getHttpStream(item.url, opts), type: 'remote' };
        } else {
            console.log(`Creating read stream for local file: ${item.url}`);
            return { stream: fs.createReadStream(item.url), type: 'file' };
        }
    }
    console.error('Invalid or missing URL:', item);
    throw new Error('URL inválido ou ausente');
};
